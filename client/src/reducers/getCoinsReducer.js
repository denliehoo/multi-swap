...
const getCoins = () => {
    return async (dispatch) => {
        ...
        const coinNameHolder = await res.data.map((i) => {
            coinNameHolder[i.symbol] = i.id;
            return
        })
        const coinTickerArrayHolder = res.data.map((i) => {
            return i.symbol.toUpperCase()
        })
        await dispatch(getCoinsHandler({
...
// the reducer
const getCoinsReducer = (state = initialState, action) => {
    switch (action.type) {
        case "GETCOINS":
            return { ...state, coinName: action.payload.coinName, coinTickerArray: action.payload.coinTickerArray, isLoading: false };
