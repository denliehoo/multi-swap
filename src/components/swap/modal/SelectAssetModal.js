...
const ethDefaultAssetInfo = [ // refactor this to an external file eventually
    {
        symbol: 'ETH',
        name: 'Ethereum',
...
const SelectAssetModal = ({ props, ethCustomTokens }) => {
    ...
    const getCombinedListOfAssets = (chain) => {
        ...
        const formattedCustomTokens = customTokens.map((i) => ({
            symbol: i.symbol, name: i.name, imgUrl: i.logo, address: i.address, isDefaultAsset: false,
                        ...
                        <div>
                            ...
                            <div>
                                <div style={{ overflow: 'auto', height: '30vh' }}>
                                    {
                                        getCombinedListOfAssets('eth').map((i) => <SelectAssetItem
                                            icon={<IconComponent imgUrl={i.imgUrl} />} symbol={i.symbol} name={i.name}
                                            balance={i.bal}
