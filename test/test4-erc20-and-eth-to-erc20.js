// truffle test test/test4-erc20-and-eth-to-erc20.js
...
const percentForEachTokenOut = ['5000', '5000'];
// 2 ftm value
...
contract(
  'Multiswap: Test case for 1. Swap ETH to ERC20 and 5. ERC20 + ETH to ERC20',
  (accounts) => {
    let instance;
    ...
    describe('Swap ETH for multiple', async () => {
      it('Get amounts out', async () => {
        const getAmountsOut =
          await instance.getAmountsOutEthForMultipleTokensByPercent(
            nativeAmount,
            poolAddressesInInitial,
            ['5000', '5000'],
          );
      ...
      it('Tokens should approve', async () => {
        ...
        await daiContract.methods
          .approve(instance.address, uint256MaxAmount)
          .send({ from: userWalletAddress });
        for (const i in poolAddressesInInitial) {
          const allowance = await instance.allowanceERC20(
            poolAddressesInInitial[i],
          );
    ...
    describe('Check For Swap Multiple Tokens And Eth For Multiple Tokens', async () => {
      it('Get amounts out', async () => {
        amountForEachTokensIn.push(nativeAmount); // swapping an extra 2 ftm on top of the erc20 tokens
        const getAmountsOut =
          await instance.getAmountsOutTokensAndEthForMultipleTokensByPercent(
            poolAddressesIn,
            amountForEachTokensIn,
            poolAddressesOut,
      ...
      it('Swap Should Work', async () => {
        const receipt2 =
          await instance.swapTokensAndEthForMultipleTokensByPercent(
            poolAddressesIn,
            amountForEachTokensIn,
            poolAddressesOut,
            percentForEachTokenOut,
